...
describe("buildInjectionSnippet", () => {
	test("includes both languages and guidelines when present", () => {
		const cache = new Map<string, GuidelineEntry>([
			...
			[
				...
				{
					file: "docs/typescript-guidelines.md",
					content: "# TypeScript\n...",
					topics: ["typescript"],
				},
			],
	...
	test("handles guidelines but no languages", () => {
		const cache = new Map<string, GuidelineEntry>([
			[
				"testing.md",
				{ file: "docs/testing.md", content: "# Testing", topics: ["testing"] },
			],
		]);
...
describe("discoverGuidelines", () => {
	...
	test("discovers language-guidelines.md files", () => {
		...
		mkdirSync(join(dir, "docs"), { recursive: true });
		writeFileSync(
			join(dir, "docs", "typescript-guidelines.md"),
			"# TypeScript guidelines\n\nUse interfaces.",
		);
		writeFileSync(
			join(dir, "docs", "python-conventions.md"),
			"# Python conventions\n\nUse snake_case.",
		);
	...
	test("discovers topic-practices.md files", () => {
		...
		mkdirSync(join(dir, "docs"), { recursive: true });
		writeFileSync(
			join(dir, "docs", "mocking-practices.md"),
			"# Mocking\n\nUse unittest.mock.",
		);
