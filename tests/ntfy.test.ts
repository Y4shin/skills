...
describe("ntfy config reader", () => {
  ...
  test("returns null when ntfy is disabled", () => {
    ...
    writeFileSync(p, JSON.stringify({ ntfy: { enabled: false, serverUrl: "https://ntfy.sh", topic: "test" } }));
  ...
  test("returns null when no topic", () => {
    const dir = mkTmp();
    const p = join(dir, "config.json");
    writeFileSync(p, JSON.stringify({ ntfy: { enabled: true, serverUrl: "https://ntfy.sh" } }));
    expect(readNtfyConfig(p)).toBeNull();
  });
  ...
  test("parses valid ntfy config", () => {
    const dir = mkTmp();
    const p = join(dir, "config.json");
    writeFileSync(p, JSON.stringify({ ntfy: { enabled: true, serverUrl: "https://ntfy.example.com", topic: "my-topic", token: "tk_12345", priority: 4 } }));
