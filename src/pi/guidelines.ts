...
export function collectSourceLanguages(cwd: string): string[] {
...
export function discoverGuidelines(cwd: string): Map<string, GuidelineEntry> {
...
export function buildInjectionSnippet(
	...
	const lines: string[] = ["## Project coding guidelines", ""];
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("tool_result", async (event, _ctx) => {
		if (event.toolName === "bash") {
			...
			if (
				...
				input.command.includes("state.yaml")
			) {
				stateChangedSinceLastInject = true;
			}
		}
	...
	pi.registerTool({
		...
		description: "List all available coding guideline sources in this project.",
