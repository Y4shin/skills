---
description: Fetch a slice issue (#n), present a structured summary, then run a focused grilling session to decide the test strategy before any code. Appends a confirmed Test plan to the issue's committed slice doc and hands off to /implement-issue. Use when starting work on an issue. Provider-aware (gh, fgj, local).
---
...
Commit the slice doc. Then hand off: invoke `/implement-issue <n>`.
