...
function artToRecord(a) {
  const data = { path: a.path, kind: a.kind, slug: a.slug, status: a.status };
    ...
    tool: {
      ...
      prd_set_slices: tool({
        ...
        async execute(args, ctx) {
          ...
          a.doc.save();
          return `slices: [${a.doc.data["slices"].join(", ")}]`;
        }
      }),
      prd_resolve: tool({
      ...
      prd_slices: tool({
        ...
        async execute(args, ctx) {
          ...
          return slices.length === 0 ? "(no open slices \u2014 ready to finalize)" : slices.map((s) => `#${s.number} \u2014 ${s.slug} (${s.path})`).join("\n");
        }
      }),
...
Complete and implement all slices before finalizing.`
      ...
      prd_lint: tool({
        ...
        async execute(args, ctx) {
          const root = getRoot(ctx.directory);
          const prd = join2(root, "docs", "prd");
          if (!existsSync2(prd)) return "(no docs/prd directory)";
          const all = discoverAll(root);
          const violations = [];
          for (const a of all) {
            for (const field of ["kind", "title", "slug", "status"]) {
              if (a.doc.data[field] === void 0 || a.doc.data[field] === null) {
      ...
      prd_epic_finalizable: tool({
        ...
        async execute(args, ctx) {
          ...
          const a = resolveArtifact(root, args.selector, "epic");
          const prds = Array.isArray(a.doc.data["prds"]) ? a.doc.data["prds"] : [];
          const undone = prds.filter((p) => !p.done).map((p) => p.slug || "?");
