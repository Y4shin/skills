...
function discoverEpics(root) {
  ...
  for (const f of globChildFiles(base, "epic.md")) {
    ...
    out.push(new Artifact(f, doc.data["kind"] ?? "epic", doc));
  }
  return out;
}
function discoverPrds(root) {
  ...
  for (const f of globChildFiles(base, "prd.md")) {
    ...
    out.push(new Artifact(f, doc.data["kind"] ?? "prd", doc));
  }
  return out;
}
...
function readPrdrc() {
  ...
  const rc = join2(root, ".prdrc");
  if (!existsSync2(rc) || !statSync2(rc).isFile()) return null;
  try {
    const data = parse2(readFileSync2(rc, "utf-8"));
    ...
    return {
      provider,
      owner: (typeof forge["owner"] === "string" ? forge["owner"] : void 0) ?? "",
      repo: (typeof forge["repo"] === "string" ? forge["repo"] : void 0) ?? ""
    };
  } catch {
    return null;
  }
...
var Client = class _Client {
  ...
  _token = null;
  _labels = null;
  constructor(owner, repo, host, token) {
    this.owner = owner;
    this.repo = repo;
    this.host = host;
    if (token !== void 0) this._token = token;
  }
  static fromRepo() {
    const f = detect();
    if (f.provider !== "fgj") {
  ...
  async listLabels() {
    if (this._labels === null) {
      ...
      this._labels = Object.fromEntries(rows.map((r) => [r.name, r.id]));
    }
    return this._labels;
  }
  ...
  async close(index, comment = null) {
  ...
  async updateIssue(index, title, body) {
    const payload = {};
    if (title !== void 0 && title !== null) payload.title = title;
    if (body !== void 0 && body !== null) payload.body = body;
...
function profileText(root) {
  const p = join3(root, "docs", "prd", "profile.md");
  if (existsSync3(p) && statSync3(p).isFile()) return readFileSync3(p, "utf-8");
  return "";
}
...
function main() {
  ...
  try {
    switch (cmd) {
      case "workflow-gate":
        return workflowGate(cwd);
      case "list":
        return listArtifacts(cwd, args.slice(1));
      case "reference":
        return printReference();
      case "profile":
        return printProfile(cwd);
      case "forge":
        return forgeSnippetCmd(args[1] ?? "");
      case "forgejo":
        return forgejoCmd(args.slice(1));
      default:
...
function printReference() {
  ...
  const paths = [
    join4(process.cwd(), "docs", "artifacts.md"),
    join4(here, "..", "docs", "artifacts.md"),
    join4(here, "..", "..", "docs", "artifacts.md")
  ];
  for (const p of paths) {
    if (existsSync4(p)) {
      console.log(readFileSync4(p, "utf-8").trimEnd());
      return;
    }
  }
  console.log("(reference docs not found)");
}
function printProfile(cwd) {
  const text = profileText(findRoot(cwd));
  console.log(text || "(no profile)");
}
function forgeSnippetCmd(key) {
  if (!key || key === "keys") {
    const keys = [
      "git_type",
      "owner",
  ...
  try {
    ...
    if (e instanceof NotAGitRepo) {
      console.log("NOT_A_GIT_REPO: run `git init` first.");
      process.exit(1);
    }
    if (e instanceof UnknownForge) {
      console.log(`UNKNOWN_FORGE: remote '${e.remote}' not recognised.`);
      process.exit(0);
    }
    throw e;
  }
}
async function forgejoCmd(subargs) {
  if (subargs.length === 0) {
  ...
  function readBody(bodyFile) {
    if (!bodyFile) return "";
    const p = join4(process.cwd(), bodyFile);
    return existsSync4(p) ? readFileSync4(p, "utf-8") : bodyFile;
  }
  function readTitleFlag(args) {
    const idx = args.indexOf("--title");
    return idx >= 0 && idx + 1 < args.length ? args[idx + 1] : null;
  }
  function readBodyFlag(args) {
    const idx = args.indexOf("--body-file");
    return idx >= 0 && idx + 1 < args.length ? args[idx + 1] : null;
  }
  function readLabelFlag(args) {
    const idx = args.indexOf("--label");
    return idx >= 0 && idx + 1 < args.length ? args[idx + 1] : null;
  }
  function readMilestoneFlag(args) {
    const idx = args.indexOf("--milestone");
    return idx >= 0 && idx + 1 < args.length ? args[idx + 1] : null;
  }
  function extractN(args) {
  ...
  switch (cmd) {
    ...
    case "view": {
      ...
      if (n === null) {
        console.error("usage: forgejo view <n>");
        process.exit(1);
      }
    ...
    case "create": {
      ...
      const body = readBody(readBodyFlag(rest) ?? void 0);
    ...
    case "comment": {
      const n = extractN(rest);
      if (n === null) {
        console.error("usage: forgejo comment <n> --body <text>");
        process.exit(1);
      }
      const body = readBody(readBodyFlag(rest) ?? void 0) || "comment";
    ...
    case "close": {
      const n = extractN(rest);
      if (n === null) {
        console.error("usage: forgejo close <n> [--comment <text>]");
        process.exit(1);
      }
    ...
    case "edit": {
      const n = extractN(rest);
      if (n === null) {
        console.error("usage: forgejo edit <n> [--title <t>] [--body-file <f>]");
        process.exit(1);
      }
      const title = readTitleFlag(rest);
      const body = readBodyFlag(rest) ? readBody(readBodyFlag(rest)) : void 0;
    ...
    case "edit-labels": {
      const n = extractN(rest);
      if (n === null) {
        console.error("usage: forgejo edit-labels <n> --add-label <a> --remove-label <r>");
        process.exit(1);
      }
    ...
    case "milestone": {
      ...
      if (sub === "create" && titleIdx >= 0 && titleIdx < rest.length) {
        ...
        if (n) {
          await c.closeMilestone(n);
          console.log(`closed milestone #${n}`);
        } else {
          console.error("usage: forgejo milestone close <n>");
          process.exit(1);
        }
      } else {
    ...
    case "dep": {
      ...
      if (n === null || blocker === null) {
        console.error("usage: forgejo dep <n> --blocked-by <blocker>");
        process.exit(1);
      }
    ...
    case "set-milestone": {
      ...
      if (n === null || !msName) {
        console.error("usage: forgejo set-milestone <n> --milestone <title>");
        process.exit(1);
      }
