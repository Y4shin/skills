...
function readPrdrc() {
  ...
  let data;
  try {
    data = parse2(readFileSync2(rc, "utf-8"));
  } catch {
    return null;
  }
...
function findMilestone(data, number) {
...
function comment(root, number, text) {
  const data = load(root);
  find(data, number).comments.push(text);
  save(root, data);
}
function close(root, number, commentText = null) {
  const data = load(root);
...
function edit(root, number, title = null, body = null) {
  const data = load(root);
  const issue = find(data, number);
...
function addDependency(root, number, blocker) {
  const data = load(root);
  const issue = find(data, number);
  find(data, blocker);
...
function closeMilestone(root, number) {
  const data = load(root);
...
function setMilestone(root, number, title) {
  const data = load(root);
  const issue = find(data, number);
...
function cmdList(root, tokens) {
  ...
  for (const r of rows) {
    ...
    out(
      `${pad(r.kind, width)}  ${pad(r.slug, 28)} ${pad(status, 14)} ${pad(issue, 6)} ${r.path}`
    );
  }
}
...
function cmdEpic(root, tokens) {
  ...
  if (sub === "prds") {
    ...
    for (const p of prds) {
      ...
      out(
        `${pad(p.slug ?? "?", 28)} ${pad(p.kind ?? "?", 12)} ${pad(issue, 6)} blocked_by:${pad(blocked, 20)} ${done}`
      );
    }
...
function cmdTracker(root, tokens) {
  ...
  switch (sub) {
    ...
    case "view": {
      const { opts, positionals } = parseOpts(rest, { json: { type: "bool", dest: "json" } });
      const issue = view(root, num(need(positionals, 0, "number")));
      if (opts.json) {
        out(
          pyJsonIndent({
            number: issue.number,
...
async function cmdForgejo(tokens) {
  ...
  switch (sub) {
    ...
    case "view": {
      ...
      out(`#${issue.number} ${issue.title}  [${issue.state}]`);
      out(`labels: ${(issue.labels ?? []).map((l) => l.name).join(", ") || "-"}`);
      out("");
      out(issue.body || "");
      return;
    }
