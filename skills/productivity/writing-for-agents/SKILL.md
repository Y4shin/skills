...
object. The pointer's _wording_, not its target, decides when the agent
...
3. **Disclosed reference** is pushed out into a separate file, reached by a
   ...
   in the same folder through fully external reference that lives anywhere and
   any document can point at.
...
far down_ a piece sits, co-location decides _what sits beside it_ once there.
...
- "fast, deterministic, low-overhead" then _tight_ (a _tight_ loop).
- "a loop you believe in" then _red_, turning a fuzzy gate into a binary
