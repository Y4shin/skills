...
Tell the user the PRD path and that it's ready for `/skill:slice-prd`. Don't create issues here.
...
Then hand off each child PRD to `/skill:create-prd` serially, seeding `epic: <epic-slug>`.

## Error handling

- If the project has no `docs/prd/` directory, run `/skill:init-prd-workflow` first.
